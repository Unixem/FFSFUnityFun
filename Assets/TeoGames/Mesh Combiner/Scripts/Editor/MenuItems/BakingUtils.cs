...
namespace TeoGames.Mesh_Combiner.Scripts.Editor.MenuItems {
	public static class BakingUtils {
		public static async Task Bake(UnityEngine.GameObject obj, string folder) {
			...
			Object.FindObjectsByType<ColliderGenerator>(FindObjectsInactive.Include, FindObjectsSortMode.None)
				.ForEach(col => col.Init());
			...
			try {
				await GetTaskToWait(combiners);
			} finally {
				Debug.Log("- Reset combinables");
				...
				Debug.Log("- Remove temporary combiner");
		...
		public static void RemoveBake(UnityEngine.GameObject obj) {
			...
			obj.GetComponentsInChildren<AbstractCombinable>(true).ForEach(
				c => {
					if (c.enabled) c.UpdateVisibility();
				}
			);
		...
		private static async Task GetTaskToWait(AbstractMeshCombiner[] combiners) {
			var completed = 0f;
			var total = combiners.Length;
			...
			var tasks = combiners.Select(
				async c => {
					...
					completed++;
					Debug.Log($"-- Combiner is baked. {completed}/{total}", c);
				}
			);
