...
namespace TeoGames.Mesh_Combiner.Scripts.Editor.MenuItems {
	public static class Utils {
		...
		public static void SetReadableStatus(UnityEngine.GameObject obj, bool val) {
			Debug.Log($"Set readable status to {val} at {obj.name}", obj);

			obj.GetComponentsInChildren<MeshFilter>().ForEach(
				f => {
					try {
						Debug.Log($"-- {f.sharedMesh.name}", f);
						f.sharedMesh.SetReadable(val);
					} catch (Exception ex) {
			...
			obj.GetComponentsInChildren<SkinnedMeshRenderer>().ForEach(
				s => {
					try {
						Debug.Log($"-- {s.sharedMesh.name}", s);
						s.sharedMesh.SetReadable(val);
					} catch (Exception ex) {
						Debug.LogException(ex);
					}
				}
		...
		public static bool ClearCache(AbstractCombinable comb) {
			...
			comb.GetCache().blendShape = oldCache.blendShape;
