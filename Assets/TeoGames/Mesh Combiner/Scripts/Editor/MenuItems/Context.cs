...
namespace TeoGames.Mesh_Combiner.Scripts.Editor.MenuItems {
	public class Context : AssetModificationProcessor {
		...
		[MenuItem("CONTEXT/SkinnedMeshRenderer/Dynamic Mesh Combiner/Debug/Print UV0", false, 1001)]
		public static void PrintUV0Combinable(MenuCommand data) {
			var ren = data.context as SkinnedMeshRenderer;
			if (ren) {
				Debug.Log($"--- Draw UV0 for {ren.name}");
				ren.sharedMesh.uv.ForEach(v => Debug.Log(v));
				Debug.Log($"--- DONE");
			}
		}

		[MenuItem("CONTEXT/SkinnedMeshRenderer/Dynamic Mesh Combiner/Debug/Print Bones", false, 1001)]
		public static void PrintBones(MenuCommand data) {
			var ren = data.context as SkinnedMeshRenderer;
			if (ren) {
				Debug.Log($"--- Bones for {ren.name}");
				ren.bones.ForEach(b => Debug.Log($"  - {b.name}", b));
				Debug.Log($"--- DONE");
			}
		}
