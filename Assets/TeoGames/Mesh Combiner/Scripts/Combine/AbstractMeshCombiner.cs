...
namespace TeoGames.Mesh_Combiner.Scripts.Combine {
	public abstract class AbstractMeshCombiner : MonoBehaviour {
		...
		public abstract bool IsBaked(AbstractCombinable combinable);
		...
		[Tooltip("Define max time in ms per frame that combiner can work. Note that in case of huge meshes it still can take more time")]
		[Min(1)]
		public int maxBuildTime = 5;

		[Tooltip("Define which renderers can be used")]
		public TargetRendererType rendererTypes = TargetRendererType.MeshRenderer | TargetRendererType.SkinnerMeshRenderer;

		[Tooltip("Will bake materials into one if enabled (only lit/simple lit materials without textures are supported, see docs)")]
		...
		[Tooltip("Index format for output mesh. For mobile you might want to use UInt16")]
