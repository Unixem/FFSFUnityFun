...
namespace TeoGames.Mesh_Combiner.Scripts.Combine.MeshRendererManager {
	public class DynamicMeshRenderer : AbstractMeshRenderer {
		...
		public DynamicMeshRenderer(bool blendShapeEnabled, ShadowCastingMode shadow, IndexFormat indexFormat, GameObject obj) : base(shadow, indexFormat) {
			...
			Task.CompletedTask.WaitForUpdate().Then(() => _Renderer.updateWhenOffscreen = true).Forget();
		...
		public override async Task BuildMesh(Timer timer, bool clearCache) {
			...
			_PreviousMesh = OutMesh;
			if (_PreviousMesh) ProfilerModule.Vertices.Value -= _PreviousMesh.vertexCount;
			OutMesh = null;
