...
namespace TeoGames.Mesh_Combiner.Scripts.Combine.ChunkContainer {
    ...
    public class GridChunkContainer : AbstractChunkContainer {
        ...
        public override float Compability(Vector3 pos) => 1f;
        ...
        public override bool IsBaked(AbstractCombinable combinable, string key) {
            return key == null 
                ? _Cells.Any(c => c.Value.IsBaked(combinable)) 
                : GetCell(key).IsBaked(combinable);
        }
        ...
        public override void PostBakeAction() {
            _Cells.ForEach(cell => { DestroyImmediate(cell.Value); });
            _Cells.Clear();
        }

        public override Task UpdateTask => Task.WhenAll(
            _Cells.Select(c => c.Value is IAsyncCombiner async ? async.UpdateTask : null)
        );
        ...
        protected override void OnVisibilityChanged() => _Cells.Values.ForEach(c => c.IsVisible = IsVisible);
    }
}