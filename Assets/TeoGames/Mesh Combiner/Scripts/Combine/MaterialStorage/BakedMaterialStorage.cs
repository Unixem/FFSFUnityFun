...
namespace TeoGames.Mesh_Combiner.Scripts.Combine.MaterialStorage {
	...
	public class BakedMaterialStorage : AbstractMaterialStorage {
		...
		public override MaterialType Get(long mID, Material mat, int offset, ShadowCastingMode shadow, bool isStatic, IndexFormat format) {
			...
			if (!_Materials.TryGetValue(bakedId, out var material)) {
				material = GetMaterialInstance(shadow, isStatic, bakedMat, format);
