...
namespace TeoGames.Mesh_Combiner.Scripts.Combine.Lod {
	...
	public class LodConfiguration {
		...
		public void Init(AbstractMeshCombiner original) {
			...
			if (original is ICombinerVisibilityTogglable v) _Combiner.chunk.IsVisible = v.IsVisible;
			...
			if (original is IRenderListener listener) {
				_Combiner.chunk.OnRenderersUpdated += list => listener.OnRenderersUpdated?.Invoke(list);
			}
		}
