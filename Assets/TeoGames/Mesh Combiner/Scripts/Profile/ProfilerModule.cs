...
namespace TeoGames.Mesh_Combiner.Scripts.Profile {
#if DISABLE_PROFILE || !UNITY_2020_3_OR_NEWER
	using System;
	...
	public static class ProfilerModule {
		...
		public static readonly ProfilerCounterValue<long> BatchMeshBakerTime = new ProfilerCounterValue<long>(
			Category,
			"Batch Mesh Baker Time",
			ProfilerMarkerDataUnit.TimeNanoseconds,
			ProfilerCounterOptions.FlushOnEndOfFrame | ProfilerCounterOptions.ResetToZeroOnFlush
		);

		public static readonly ProfilerCounterValue<long> UpdateQueueTime = new ProfilerCounterValue<long>(
			Category,
			"Update Queue Time",
			ProfilerMarkerDataUnit.TimeNanoseconds,
			ProfilerCounterOptions.FlushOnEndOfFrame | ProfilerCounterOptions.ResetToZeroOnFlush
		...
		private static void OnEnterPlaymodeInEditor() {
			...
			BakeTime.Value = 0;
			UpdateQueueTime.Value = 0;
			BatchMeshBakerTime.Value = 0;
		}
#endif
	}
}