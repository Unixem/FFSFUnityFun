...
namespace TeoGames.Mesh_Combiner.Scripts.Extension {
	public static class MeshBonesExtension {
		...
		static MeshBonesExtension() => StaticCache.Register(ResetCache, ValidateCache);

		private static void ValidateCache() {
			var before = OptimizedMeshes.Count;
			if (before == 0) return;

			var keys = new Mesh[before];
			OptimizedMeshes.Keys.CopyTo(keys, 0);

			for (var i = 0; i < before; i++) {
				if (keys[i]) continue;
				if (OptimizedMeshes.TryGetValue(keys[i], out var val) && val.mesh)
					Object.Destroy(val.mesh);
				OptimizedMeshes.Remove(keys[i]);
			}
		}
