...
namespace TeoGames.Mesh_Combiner.Scripts.Extension {
	public static class MeshExtension {
		...
		static MeshExtension() => StaticCache.Register(ResetCache, ValidateCache);

		private static void ValidateCache() {
			PruneCache(ToStaticCache);
			PruneCache(ToAnimatedCache);
		}

		private static void PruneCache(Dictionary<Mesh, Mesh> cache) {
			var count = cache.Count;
			if (count == 0) return;

			var keys = new Mesh[count];
			cache.Keys.CopyTo(keys, 0);

			for (var i = 0; i < count; i++) {
				if (keys[i]) continue;
				if (cache.TryGetValue(keys[i], out var val) && val)
					Object.Destroy(val);
				cache.Remove(keys[i]);
			}
		}
