...
namespace TeoGames.Mesh_Combiner.Scripts.Extension {
	public static class EnumerableExtension {
		...
		public static T[] GetRandomElements<T>(this T[] array, int n) {
			...
			if (n == 1) return new[] { array[Random.Range(0, array.Length)] };

			var pool = new T[array.Length];
			Array.Copy(array, pool, array.Length);

			for (var i = 0; i < n; i++) {
				var j = Random.Range(i, pool.Length);
				(pool[i], pool[j]) = (pool[j], pool[i]);
			}

			var result = new T[n];
			Array.Copy(pool, result, n);
			return result;
		}
