...
namespace Akila.FPSFramework
{
    ...
    public class FirstPersonController : MonoBehaviour, ICharacterController
    {
        ...
        private RaycastHit[] feetHits;
        private int feetHitsCount;
        ...
        private void MoveWithMovingPlatforms()
        {
            ...
            feetHitsCount = Physics.SphereCastNonAlloc(feetPosition, controller.radius, Vector3.down, feetHits, controller.radius / 2);

            // Loop through all hits to calculate total velocity from Speedometer components
            for (int i = 0; i < feetHitsCount; i++)
            {
                if (feetHits[i].transform != transform) // Ensure we're not processing the character itself
                {
                    Vector3 hitVelocity = GetTransformVelocity(feetHits[i].transform);
