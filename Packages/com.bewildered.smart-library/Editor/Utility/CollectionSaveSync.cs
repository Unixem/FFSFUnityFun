...
namespace Bewildered.SmartLibrary
{
    ...
    internal static class CollectionSaveSync
    {
        ...
        static CollectionSaveSync()
        {
            ...
            LibraryDatabase.ItemsChanged += OnCollectionItemsChanged;
        ...
        private static void OnCollectionItemsChanged(LibraryItemsChangedEventArgs args)
        {
            EnqueueCollection(args.collection);
            
            // We enqueue the root collection to be saved for any item changes because it contains a list of all items.
            // If a collection does not register an undo with the root collection (done via PrepareItemsChange), then
            // the it will not trigger a ObjectChanged event and will not be saved.
            
            // For example the SmartCollection does not register an undo because
            // it s meant to by synced at all times with the asset database.  
            if (args.collection.Root != null)
                EnqueueCollection(args.collection.Root);
        }
