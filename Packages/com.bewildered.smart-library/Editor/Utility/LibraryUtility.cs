    ...
    internal static class LibraryUtility
    {
        ...
        internal static void LoadAllCollections()
        {
            ...
            if (rootCollection == null)
            {
                ...
                idCollectionPairs[rootCollection.ID] = rootCollection;
            ...
            foreach (var collection in LibraryDatabase.EnumerateCollections())
            {
                ...
                if (collection.Parent == null)
                {
                    ...
                    Debug.LogWarning($"[SmartLibrary] Could not find parent with id '{parentId}' for '{collection}'. Change to a base collection.");
                }

                // Clear subcllection ids that don't have a valid collection.
                for (int i = collection.SubcollectionIDs.Count - 1; i >= 0; i--)
                {
                    if (!LibraryDatabase.ContainsID(collection.SubcollectionIDs[i]))
                    {
                        collection.SubcollectionIDs.RemoveAt(i);
                        break;
                    }
                }
