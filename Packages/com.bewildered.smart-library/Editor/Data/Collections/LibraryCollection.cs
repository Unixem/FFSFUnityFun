...
namespace Bewildered.SmartLibrary
{
    ...
    /// </summary>
    [Serializable]
    public abstract class LibraryCollection : ScriptableObject, IReadOnlyCollection<LibraryItem>
    {
        ...
        [NonSerialized] private SelectorReadonlyList<UniqueID, LibraryCollection> _subcollectionSelector;

        private SelectorReadonlyList<UniqueID, LibraryCollection> SubcollectionSelector
        {
            get
            {
                return _subcollectionSelector ??= new SelectorReadonlyList<UniqueID, LibraryCollection>(_subcollectionIDs, LibraryDatabase.FindCollectionByID);
            }
        }

        internal RootLibraryCollection Root
        {
            get { return LibraryDatabase.FindRoot(this); }
        }
        ...
        {
            get { return LibraryDatabase.FindCollectionByID(_parentID); }
            internal set
            {
                if (value != null)
                    _parentID = value.ID;
        ...
        /// </summary>
        public IReadOnlyList<LibraryCollection> Subcollections
        {
            get { return SubcollectionSelector; }
        }
        ...
        public Transform DefaultSceneParent
        {
            get
            {
                if (_defaultParentRef == null)
                {
                    ...
                    var defaultParents = FindObjectsByType<CollectionDefaultParent>();
        ...
        {
            InsertSubcollection(_subcollectionIDs.Count, collection);
        }
        ...
        public void RemoveSubcollection(LibraryCollection collection)
        {
            ...
            if (!_subcollectionIDs.Contains(collection.ID) || collection == this)
                return;
            ...
            int index = _subcollectionIDs.IndexOf(collection.ID);
        ...
        private bool IsValidReparenting(LibraryCollection newParent)
        {
            ...
            if (newParent._subcollectionIDs.Contains(_id))
                return false;
        ...
        public int GetSiblingIndex()
        {
            if (Parent == null)
                return -1;
            return Parent._subcollectionIDs.IndexOf(_id);
        }
        ...
        public void MoveSubcollection(int newIndex, int oldIndex)
        {
            ...
            var collectionId = _subcollectionIDs[oldIndex];
            ...
            _subcollectionIDs.Insert(newIndex, collectionId);
            FinishChange();

            NotifySubcollectionsChanged(LibraryDatabase.FindCollectionByID(collectionId), oldIndex, HierarchyChangeType.Moved);
        }
       ...
       public static LibraryCollection CreateCollection<T>()
        ...
        public static void DestroyCollection(LibraryCollection collection)
        {
            ...
            for (int i = collection._subcollectionIDs.Count - 1; i >= 0; i--)
            {
                DestroyCollection(LibraryDatabase.FindCollectionByID(collection._subcollectionIDs[i]));
            }
        ...
        public static LibraryCollection DuplicateCollection(LibraryCollection collection, LibraryCollection parent)
        {
            ...
            for (int i = 0; i < collection._subcollectionIDs.Count; i++)
            {
                DuplicateCollection(LibraryDatabase.FindCollectionByID(collection._subcollectionIDs[i]), copy);
            }
