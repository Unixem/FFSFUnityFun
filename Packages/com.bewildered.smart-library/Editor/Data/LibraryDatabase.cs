...
{
    /// <summary>
    /// An interface for accessing collections and items in the library and performing operations on the library.
    /// </summary>
    public static class LibraryDatabase
    {
        ...
        /// </summary>
        public static IReadOnlyList<LibraryCollection> BaseCollections
        {
        ...
        internal static RootLibraryCollection FindRoot(LibraryCollection collection)
        {
            var parent = collection;
            while (parent != null && !(parent is RootLibraryCollection))
            {
                parent = parent.Parent;
            }
            
            // Will either be the root, or will return null when cast fails.
            return parent as RootLibraryCollection;
        }
        ...
        public static bool ContainsID(UniqueID id)
        {
            return SessionData.instance.IDToCollectionMap.ContainsKey(id);
        }
        ...
        public static IEnumerable<T> GetAllCollectionsOfType<T>() where T : LibraryCollection
        {
            if (SessionData.instance.TypeCollectionPairs.TryGetValue(typeof(T),
                    out List<LibraryCollection> collections))
            {
                foreach (var collection in collections.Cast<T>())
                {
