...
namespace Bewildered.SmartLibrary
{
    ...
    public class PreviewRenderer
    {
        ...
        {
            // At some pointer after first render HDRP resets the intensity of the light to 10,000 and blows out the render.
            // So we have to just set it each time we render.
#if HDRP_1_OR_NEWER
            _stage.MainLight.intensity = 4f;
#endif
        ...
        private void InitializeNewRenderTexture()
        {
            ...
            var rtd = new RenderTextureDescriptor(size, size)
            {
                depthBufferBits = 24,
                msaaSamples = 1,
                useMipMap = false, sRGB = true, graphicsFormat = format
            };
